<?php
/**
 * Fetches and processes episode data from the external MeloloAPI.
 * @param int $post_id The ID of the post.
 * @return array An array containing 'episodes' and 'details'.
 */
function _shortplyr_fetch_api_episodes($post_id) {
    $episodes_data = [];
    $book_details = [];
    ...
    return ['episodes' => $episodes_data, 'details' => $book_details];
}

/**
 * Gets episode data from post meta (manual mode).
 * @param int $post_id The ID of the post.
 * @return array The episode data.
 */
function _shortplyr_get_manual_episodes($post_id) {
    $episodes_data = [];
...
/**
 * Injects ad data into the episodes list.
 * @param array $episodes The list of episodes.
 * @param int $post_id The ID of the post.
 * @return array The modified list of episodes.
 */
function _shortplyr_inject_ads($episodes, $post_id) {
    ...
    foreach ($ad_indices as $index) {
        if (isset($episodes[$index])) {
            $episodes[$index]['is_ad'] = true;
            $episodes[$index]['ad_src'] = $adsterra_url;
            unset($episodes[$index]['src']);
        }
    }
    return $episodes;
}
...
/**
 * Determines the final poster URL based on settings.
 * @param string $data_mode The data mode ('api' or 'manual').
 * @param array $api_book_details The details from the API.
 * @param int $post_id The ID of the post.
 * @return string The final poster URL.
 */
function _shortplyr_determine_poster($data_mode, $api_book_details, $post_id) {
    $api_poster_url = $api_book_details['thumb_url'] ?? '';
    ...
    if ($data_mode === 'manual' && $choice === 'api') {
        $choice = 'metabox'; // Force choice to metabox if data mode is manual
    }

    $final_poster_url = '';
    switch ($choice) {
        case 'metabox': $final_poster_url = $custom_poster_url; break;
        case 'featured': $final_poster_url = $featured_image_url; break;
        case 'api':
        default: $final_poster_url = $api_poster_url; break;
    }

    // Fallback logic
    if (empty($final_poster_url)) {
        $final_poster_url = $api_poster_url ?: $custom_poster_url ?: $featured_image_url ?: '';
    }
...
function get_processed_serial_data($post_id) {
    ...
    if ($data_mode === 'manual') {
        $episodes_data = _shortplyr_get_manual_episodes($post_id);
        ...
        $episodes_data = $api_data['episodes'];
    ...
    $episodes_with_ads = _shortplyr_inject_ads($episodes_data, $post_id);
