(function () {
  ...
  const showSkeleton = (show = true) =>
    playerSkeleton?.classList.toggle('hidden', !show);
  ...
  function dragEnd() {
    ...
    const deltaPx = drag.lastY - drag.startY;
    const v = drag.velocity;
    let target = SHEET_MID_Y;
    if (v <= -VELOCITY_TRIGGER || deltaPx <= -SWIPE_THRESHOLD_PX) {
      target = SHEET_TOP_Y;
    } else if (v >= VELOCITY_TRIGGER || deltaPx >= SWIPE_THRESHOLD_PX) {
      target = SHEET_CLOSED_Y;
    } else {
      target =
        Math.abs(currentY - SHEET_TOP_Y) < Math.abs(currentY - SHEET_MID_Y)
          ? SHEET_TOP_Y
          : SHEET_MID_Y;
    }
    setY(target);
    isOpen = target !== SHEET_CLOSED_Y;
    drag.active = false;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    try {
      if (typeof seriesData === 'undefined') {
        showError('Data serial tidak ditemukan.');
        return;
      }
