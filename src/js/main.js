...
// (Kode aplikasi ada di bawah, tidak perlu impor app.js yang duplikat)
...
(function () {
  ...
  const handleEpisodeSelection = (n) => {
    ...
    if (episodeData.is_ad) {
        const storageKey = `clickedAds_${window.seriesData.id}`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    try {
      if (typeof window.seriesData === 'undefined') { showError("Data serial tidak ditemukan."); return; }
      SERIES_TITLE = window.seriesData.title || "Tanpa Judul";
      TOTAL = Number(window.seriesData.total) || 0;
      EPISODES = Array.isArray(window.seriesData.episodes) ? window.seriesData.episodes : [];
      setY(SHEET_CLOSED_Y);
